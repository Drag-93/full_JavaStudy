...
public class ItemUpdateServiceImpl implements ItemService{
	@Override
	public void excuteQueryService() {
		...
		while(true) {
			System.out.println("상품수정 실행(q면 종료)");
			String q=input.next();
			if(q.equals("q")) {
			...
			ItemDto dto=new ItemDto(itemId, itemTitle, itemDetail, price, inventory, registrator, null, LocalDateTime.now());
			int rs=dao.saveUpdate(dto);
			
//			int rs=dao.saveUpdate(itemId, itemTitle, itemDetail, price, inventory, registrator,null, LocalDateTime.now());
